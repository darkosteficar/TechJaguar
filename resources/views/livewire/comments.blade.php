<div class="antialiased mx-auto w-3/4 ">
    <h3 class="mb-4 text-xl font-semibold text-green-400">Comments</h3>
        @if (session('message'))
            <div class="bg-green-500 text-white border border-gray-900 py-2 px-2 rounded full mb-2 flex justify-between">
                <p>{{ session('message') }}</p>
                <p>{{ session('time') }}</p>
      ...
      <div class="flex mx-auto items-center justify-center shadow-lg   mb-4 ">
        ...
        @endauth
        @guest
        <div class="bg-gray-800 text-green-400 shadow-2xl flex justify-between w-full items-center px-2 rounded-sm">
            <p class="inline-block text-gray-300">You need to be logged in to be able to post comments!</p>
            
            <p class="text-lg hover:text-white border border-transparent hover:border-green-400 p-1"><a href="{{ route('login', []) }}">Login here</a></p>
        </div>
